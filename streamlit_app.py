...
def render_sidebar_inputs() -> dict:
    ...
    future_growth_rate = st.sidebar.slider("未來薪資年成長率估算", 0.0, 0.08, 0.0, 0.005)
    ...
    account_return_rate = st.sidebar.slider("勞退帳戶年化收益率假設", 0.0, 0.06, 0.011473, 0.001)
    existing_labor_pension_balance = st.sidebar.number_input("目前已累積勞退個人專戶餘額", 0, 50_000_000, 0, 10_000)
    estimate_past_labor_pension = st.sidebar.checkbox(
        "不知道目前專戶餘額，改由工具估算過去累積",
        value=False,
        help="若已輸入勞保局查到的目前專戶餘額，請不要勾選，避免把過去提繳重複計入。",
    )
    ...
    return {
        ...
        "estimate_past_labor_pension": bool(estimate_past_labor_pension),
...
def render_projection(projection: PensionProjection, inputs: dict) -> None:
    ...
    st.info("若要和勞保局『勞工保險老年給付』試算比較，請只看左下的「勞保老年年金（月領）」。本頁的合計每月現金流另加了勞退專戶月攤提，口徑不同。")
    ...
    with st.expander("勞退新舊制明細", expanded=True):
        ...
        st.write(
            f"勞退月退休金用期初年金現值因子 {projection.labor_pension_annuity_factor:.4f} "
            f"估算，利率採目前公告口徑 1.1473%。"
        )
        if inputs["estimate_past_labor_pension"]:
            st.warning("目前設定為由工具估算過去已累積勞退專戶。若你已從勞保局查到目前專戶餘額，請取消此勾選，否則會偏高。")
...
| 是否估算過去勞退累積 | 不知道目前專戶餘額時才使用 | 已輸入實際餘額時不要勾選 |
...
- 勞保局 e 化服務會使用你的實際投保紀錄，本工具只能用你輸入的起點、目前值與未來假設推估。
- 舊制基數乘數以退休前5年平均月薪估算，非僅取最後一年。
- 勞退專戶若已輸入目前實際餘額，請不要再勾選估算過去累積，否則會重複計入。
"""
    )
    st.info(DISCLAIMER)
